	...
	for i := 0; i < length; {
		...
		if c == '%' {
			...
			case 'Y': //四位数的年份表示（0000-9999）
				core.ItoAW(&theTime, my.year, 4)
			case 'y': //两位数的年份表示（00-99）
				core.ItoAW(&theTime, my.year, 2)
			case 'm': //月份（01-12）
				core.ItoAW(&theTime, my.month, 2)
			case 'd': //月内中的一天（0-31）
				core.ItoAW(&theTime, my.day, 2)
			case 'H': //24小时制小时数（0-23）
				core.ItoAW(&theTime, my.hour, 2)
			case 'M': //分钟数（00=59）
				core.ItoAW(&theTime, my.min, 2)
			case 'S': //秒（00-59）
				core.ItoAW(&theTime, my.sec, 2)
			default:
