...
func checkClock(hour, min, sec int) error {
	if hour > 23 || hour < 0 {
		return errors.Errorf("check date clock error: out of range hour=%v", hour)
	}
	if min < 0 || min > 59 {
		return errors.Errorf("check date clock error: out of range min=%v", min)
	}
	if sec < 0 || sec > 59 {
		return errors.Errorf("check date clock error: out of range sec=%v", sec)
	}
	return nil
}
...
func checkDateClock(year, month, day, hour, min, sec int) error {
	if year > 9999 || year < 1 {
		return errors.Errorf("check date clock error: out of range year=%v", year)
	}
	if month > 12 || month < 1 {
		return errors.Errorf("check date clock error: out of range month=%v", month)
	}
	if day < 1 || day > 31 {
		return errors.Errorf("check date clock error: out of range day=%v", day)
	}
	...
	for i := 0; i < length; {
		...
		if c == '%' {
			...
			case 'Y': //四位数的年份表示（0000-9999）
				core.ItoAW(&theTime, year, 4)
			case 'y': //两位数的年份表示（00-99）
				core.ItoAW(&theTime, year, 2)
			case 'm': //月份（01-12）
				core.ItoAW(&theTime, month, 2)
			case 'd': //月内中的一天（0-31）
				core.ItoAW(&theTime, day, 2)
			case 'H': //24小时制小时数（0-23）
				core.ItoAW(&theTime, hour, 2)
			case 'M': //分钟数（00=59）
				core.ItoAW(&theTime, min, 2)
			case 'S': //秒（00-59）
				core.ItoAW(&theTime, sec, 2)
			default:
...
func DateClockToUnix(year, month, day, hour, min, sec int, zone TimeZone) (unix int64, yDay int, daySecond int, err error) {
	...
	if err != nil {
		err = errors.Errorf("date clock to unix error check failed: %v, %v", DateClockToYmdHMS(year, month, day, hour, min, sec), err)
		return
	}
...
func FormatToDateClock(s, formatter string, extend bool) (year, month, day, hour, min, sec int, err error) {
	...
	for i := 0; i < length; {
		...
		if c == '%' {
			...
			default:
				err = errors.Errorf("format to date clock error char: %v, %v, %v", c, c2, formatter)
				return
			}

			if !found {
				err = errors.Errorf("format to date clock error time: %v, %v", s, formatter)
				return
			}
			jump = 0
			i += 2
			...
			if !extend {
				...
				if (fnd && byt != c) || !fnd {
					err = errors.Errorf("format to date clock error separator: %v, %v", s, formatter)
					return
				}
			}
			jump += 1
	...
	err = checkDateClock(year, month, day, hour, min, sec)
	if err != nil {
		err = errors.Errorf("format to date clock error check failed: %v, %v", s, err)
	}
	return
}
...
// @param:       zone TimeZone "时区"
// @return:      int(1-53) "第几周"
func UnixYearWeekNumAByISO(unix int64, zone TimeZone) (int, int) {
...
func UnixDayZeroHourNextSec(unix int64, sec int, zone TimeZone) int64 {
	start := UnixDayZeroHour(unix, zone)
...
func UnixNextWeekDayA(unix int64, week int, hour, min, sec int, zone TimeZone) (int64, error) {
	if week < 1 || week > 7 {
		return 0, errors.Errorf("week out of range(1,7): %v", week)
	}
...
func UnixNextWeekDayB(unix int64, week, hour, min, sec int, zone TimeZone) (int64, error) {
	if week < 0 || week > 6 {
		return 0, errors.Errorf("week out of range(0, 6): %v", week)
	}
...
// @param:       week 1-7
// @return:      int64 "秒级时间戳"
func UnixWeekDayZeroHourA(unix int64, week int, zone TimeZone) (int64, error) {
	if week < 1 || week > 7 {
		return 0, errors.Errorf("week out of range(1, 7): %v", week)
	}
...
var hmsLenErr = errors.New("hms string len error")
...
// 开始时间和结束时间相差月份
func UnixDiffMonth(start, end int64, offset int, zone TimeZone) int {
	d1 := UnixToDateTime(start-int64(offset), zone)
	d2 := UnixToDateTime(end-int64(offset), zone)
	return DateTimeDiffMonth(d1, d2)
}

// 开始时间和结束时间相差天数
func UnixDiffDay(start, end int64, offset int, zone TimeZone) int {
	unixDayStart := int(UnixDayStartTime(start-int64(offset), zone))
	unixDayEnd := int(UnixDayStartTime(end-int64(offset), zone))
	return (unixDayEnd - unixDayStart) / daySec
}
...
func SeasonId(startUnix int64, unix int64, monthNum, offset int, zone TimeZone) int {
	...
	monthCha := UnixDiffMonth(startUnix, unix, offset, zone)
	if monthCha > 0 {
...
// offset 偏移量 秒数 最大3600*24*28
func SeasonDateTime(startUnix int64, seasonId, monthNum, offset int, firstZeroTime bool, zone TimeZone) (begin *DateTime, end *DateTime) {
